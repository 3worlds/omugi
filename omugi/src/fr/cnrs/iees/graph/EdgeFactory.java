 ...
 * the creation of an edge is appropriately reflected in node instances (e.g. in {@link fr.cnrs.iees.graph.impl.ALNode ALNode},
 * by inserting the edge in its start node and end node edge lists).</p>
 * 
 * <p>Implementing classes must have a constructor taking a {@linkplain fr.cnrs.iees.identity.IdentityScope scope} as a parameter, or
...
public interface EdgeFactory {
	...
	/** A default edge identifier (e.g. to start a {@link fr.cnrs.iees.identity.impl.LocalScope LocalScope})*/
	...
	public default Class<? extends Edge> edgeClass(String label) {
		try {
			return (Class<? extends Edge>) Class.forName("fr.cnrs.iees.graph.impl."+label,false,OmugiClassLoader.getAppClassLoader());
