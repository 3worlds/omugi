...
import fr.cnrs.iees.io.parsing.impl.TreeProperties;
...
import fr.cnrs.iees.playground.graphs.ITree;
import fr.cnrs.iees.playground.io.TreeTokenizer2.treeToken;
 ...
 *
 */
public class TreeParser2 extends MinimalGraphParser2 {

	private Logger log = Logger.getLogger(TreeParser2.class.getName());
	...
	private TreeTokenizer2 tokenizer;
	
	// the result of this parsing
	private ITree<? extends ITreeNode> tree;
	...
	private ITreeNodeFactory treeFactory = null;
	

	public TreeParser2(TreeTokenizer2 tokenizer,  ITreeNodeFactory treeNodeFactory) {
		super();
		this.tokenizer =tokenizer;
		this.treeFactory = treeNodeFactory;
	}
	...
	private void buildTree() {
		// create the node but cant node.addProperty(property) since the list is not necessarily extendable 
		if (lastNodes==null)
			parse();
		Class<? extends ITree<? extends ITreeNode>> treeClass = null;
		Class<? extends ITreeNodeFactory> tFactoryClass = null;
...
//					getClass(TreeProperties.CLASS,MutableTreeImpl.class.getName());
		...
		// make tree nodes
		Map<String,ITreeNode> nodes = new HashMap<>();
		for (treeNodeSpec ns:nodeSpecs) {
			ITreeNode n = null;
			Class<? extends ITreeNode> nc = treeFactory.treeNodeClass(ns.label);
			if (ns.props.isEmpty())
				if (nc==null)
					n = treeFactory.makeNode(name);
				else
			...
			if (ns.parent!=null) {
				...
				ITreeNode parent = nodes.get(parentId); // parent has always been treated before
				n.setParent(parent);
				parent.addChild(n);
			}
		}
		...
		try {
			...
			tree = (ITree<? extends ITreeNode>) cons.newInstance(nodes.values());
		} catch (Exception e) {
	 ...
	 */
	public ITree<? extends ITreeNode> graph() {
		if (tree==null)
			buildTree();
		return tree;
	}
