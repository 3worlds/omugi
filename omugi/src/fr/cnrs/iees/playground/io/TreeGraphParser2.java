...
import fr.cnrs.iees.io.parsing.impl.GraphProperties;
...
import fr.cnrs.iees.playground.graphs.TreeGraph2;
...
public class TreeGraphParser2 extends MinimalGraphParser2 {

	private Logger log = Logger.getLogger(TreeGraphParser2.class.getName());
	...
	private TreeGraphTokenizer2 tokenizer = null;
	...
	private TreeGraph2<? extends ITreeGraphNode,? extends IEdge> graph = null;
	 ...
	 * @param edgeFactory 
	 */
	public TreeGraphParser2(TreeGraphTokenizer2 tokenizer, IEdgeFactory edgeFactory, ITreeNodeFactory treeNodeFactory) {
		super();
...
//		propertyListFactory = graphFactory;
		// make tree nodes
		Map<String,ITreeGraphNode> nodes = new HashMap<>();
		for (treeNodeSpec ns:nodeSpecs) {
			ITreeGraphNode n = null;
			Class<? extends ITreeGraphNode> nc = 
				(Class<? extends ITreeGraphNode>) graphFactory.nodeClass(ns.label);
			if (ns.props.isEmpty()) 
				if (nc==null)
	...
	@Override
	public TreeGraph2<? extends ITreeGraphNode,? extends Edge> graph() {
		if (graph==null)
			buildGraph();
		return graph;
	}
