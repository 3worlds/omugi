...
import fr.cnrs.iees.playground.io.GraphTokenizer2.graphToken;

/**
 * <p>
 * A replacement parser for Shayne's 'UniversalParser'. Simpler. Maybe Faster.
 * Who am I to pretend it's better.
 * </p>
 * 
 * <p>
 * This parser is initialised with a {@link GraphTokenizer}, i.e. it gobbles a
 * list of tokens and spits out a {@link Graph} when asked for it. It is lazy,
 * i.e. it will not do anything until asked for a graph, i.e. invoking the
 * {@code GraphParser.graph()} method, and it will parse only once after
 ...
 * 
 * <p>
 * Options to setup the graph may be passed through graph-level properties in
 * the file. These are found in {@link GraphProperties}. The best way to go is
 * to implement specific {@link NodeFactory}, {@link EdgeFactory} and
 * {@link PropertyListFactory} which will implement which flavour of
 * {@link Node}, {@link Edge} and property list (cf. {@link PropertyListGetters}
 * descendants) should be used to construct the graph.
 * </p>
 * 
 * <p>
...
public class GraphParser2 extends MinimalGraphParser2 {

	private Logger log = Logger.getLogger(GraphParser2.class.getName());
	...
	private GraphTokenizer2 tokenizer = null;
	...
	private IGraph<? extends INode, ? extends IEdge> graph;

	// BIG messy with immutables and construction.
	// a propertylist must have a constructor that takes all properties! ok
	// ... and so a graph needs a constructor with a node list.
	// so all this get exposed to the user
	// Therefore its no use passing in a graph instance because it can't be extended unless the node tokens are passed as a 
	// So can a token list be passed to a Graph Constructor
	// lazy init: nothing is done before it's needed
	public GraphParser2(GraphTokenizer2 tokenizer,IEdgeFactory edgeFactory, INodeFactory nodeFactory) {
		super();
	...
	/**
	 * <p>
	 * Construct the list of specifications for node and edge to build from the
	 * token list.
	 * </p>
	 * <p>
	 * Rules:
	 ...
	 * <li>A sequence of {@code (NODE_REF, LABEL, NAME, NODE_REF)} tokens specifies
	 * an {@code EDGE}</li>
	 * <li>A sequence of {@code (LABEL, NAME)} tokens specifies a {@code NODE}</li>
	 * <li>A sequence of {@code (PROPERTY_NAME, PROPERTY_TYPE, PROPERTY_VALUE)}
	 * tokens specifies a property</li>
	 * <li>A property specification applies to the last item ({@code NODE} or
	 * {@code EDGE}) found</li>
	 * <li>Properties found at the top of the list, before any edge or node
	 * specification, apply to the graph</li>
	...
	private void buildGraph() {
		...
		INodeFactory nFactoryClass = nodeFactory;
		IEdgeFactory eFactoryClass = edgeFactory;
		...
		for (propSpec p : graphProps) {
			...
			if (gp == null) { // all other properties are considered to be (label,class name) pairs
				...
				switch (gp) {
				case CLASS:
					graphClass = (Class<? extends Graph<? extends Node, ? extends Edge>>) getClass(
							GraphProperties.CLASS, p.value, log);
					break;
				case NODE_FACTORY:
					nFactoryClass = (Class<? extends NodeFactory>) getClass(GraphProperties.NODE_FACTORY, p.value, log);
					break;
				case EDGE_FACTORY:
					eFactoryClass = (Class<? extends EdgeFactory>) getClass(GraphProperties.EDGE_FACTORY, p.value, log);
					break;
				case PROP_FACTORY:
					plFactoryClass = (Class<? extends PropertyListFactory>) getClass(GraphProperties.PROP_FACTORY,
							p.value, log);
					break;
				case DIRECTED:
					// TODO
				...
				case MUTABLE:
					graphClass = (Class<? extends Graph<? extends Node, ? extends Edge>>) getClass(
							GraphProperties.CLASS, MutableGraphImpl.class.getName(), log);
					break;
				default:
					break;
				}
		...
		if (graphClass == null)
			graphClass = (Class<? extends Graph<? extends Node, ? extends Edge>>) getClass(GraphProperties.CLASS, log);
		if (nFactoryClass == null)
			nFactoryClass = (Class<? extends NodeFactory>) getClass(GraphProperties.NODE_FACTORY, log);
		if (eFactoryClass == null)
			eFactoryClass = (Class<? extends EdgeFactory>) getClass(GraphProperties.EDGE_FACTORY, log);
		if (plFactoryClass == null)
			plFactoryClass = (Class<? extends PropertyListFactory>) getClass(GraphProperties.PROP_FACTORY, log);
		// setup the factories
		try {
		...
		try {
			...
			graph = (Graph<? extends Node, ? extends Edge>) cons.newInstance(nodes.values()); // pbs with non empty
																								// constructors
		} catch (Exception e) {
	 ...
	 */
	public Graph<? extends INode, ? extends IEdge> graph() {
		if (graph == null)
			buildGraph();
		return graph;
	}
	...
	@Override
	public IMinimalGraph<?> graph(IEdgeFactory edgeFactory, INodeFactory nodeFactory,
